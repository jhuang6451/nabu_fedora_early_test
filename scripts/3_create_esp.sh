...
# 在任何命令失败时立即退出
set -e
# 如果在命令中使用了未定义的变量，则视为错误并退出
set -u
# 管道命令中任何一个失败，整个管道都算失败
set -o pipefail
...
# --- 清理函数 (Trap) ---
# 定义一个函数，用于在脚本退出时（无论成功、失败或被中断）执行清理操作
cleanup() {
    echo "INFO: Performing cleanup..."
    # 以相反的顺序卸载挂载点，忽略可能发生的“not mounted”错误
    umount "${ROOTFS_MNT_POINT}/boot/efi" 2>/dev/null || true
...
# 查找 vmlinuz-* 文件，-print -quit 确保只找到第一个就停止
KERNEL_VMLINUZ=$(find "$ROOTFS_MNT_POINT/boot/" -name "vmlinuz-*" -print -quit)
...
# 从完整路径中提取文件名，然后去除 'vmlinuz-' 前缀得到版本号
...
mkfs.vfat -F 32 -n "ESP" "$ESP_NAME" # -n 设置文件系统标签 (LABEL)
...
# 5.1 将 ESP 挂载点绑定到 rootfs 的 /boot/efi 目录
# 这是关键步骤，使得 chroot 环境可以访问并写入 ESP 镜像
...
# 在执行 chroot 之前，先创建目标目录，确保 dracut 不会因目录不存在而失败
